...
from sqlalchemy import Column, Integer, String, DateTime, Boolean, Float, Enum, ForeignKey, Time
...
if __name__ == "__main__":
    ...
    db.session.add(c3)
    db.session.add(c4)
    s1 = Schedule(examination_date=datetime(2021, 12, 18, 0))
    s2 = Schedule(examination_date=datetime(2021, 12, 20, 0))
    db.session.add(s1)
    db.session.add(s2)
