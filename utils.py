...
def add_new_Customer(first_name, last_name, birthday, phone_number, gender_id, appointment_date, note, ordered_date):
    #new Schedule
...
def next_sche(date_time, name=None, phone=None, address=None, date_appoinrment=None, date_exam= None, note=None):
    now = datetime.now().time()
    customer = Customer.query
    or_date = get_sat_in_date(date_time.date())
    if or_date:
        for p in or_date:
            return Schedule.query.get(p).id
        for order in or_date:
            if order.hour == or_date.hour and now.minute == order.minute:
                if name:
                    customer = Schedule.query.get(id).firsr_name
                if phone:
                    customer = Schedule.query.get(id).phone_number
                if date_appoinrment:
                    customer = Schedule.query.get(id).appointment_date()
                if date_exam:
                    customer = Schedule.query.get(id).examination_date()
                if address:
                    customer = Schedule.query.get(id).adress_id
                if note:
                    customer = Schedule.query.get(id).note
            return customer.all()
